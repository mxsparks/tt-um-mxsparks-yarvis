...
class yarvis_cmodel(pluginTemplate):
    __model__ = "yarvis_cmodel"
    __version__ = "yarvis C-model"
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.abspath(os.path.join(config['PATH'] if 'PATH' in config else "","yarvis_cmodel"))
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # for yarvis_cmodel start building the '--isa' argument. the self.isa is dutnmae specific and may not be
      # useful for all DUTs
      ...
      self.compile_cmd = self.compile_cmd+' -mabi='+('lp64 ' if 64 in ispec['supported_xlen'] else 'ilp32e ')
    ...
    def runTests(self, testList):
	  ...
	  # echo statement.
          if self.target_run:
            ...
            simcmd = self.dut_exe + ' -s {1} -g 4 -e {2}'.format(self.isa, sig_file, elf)
          else:
            simcmd = 'echo "NO RUN"'
